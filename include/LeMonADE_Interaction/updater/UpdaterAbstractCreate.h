...
--------------------------------------------------------------------------------*/
#ifndef LEMONADEINTERACTION_UPDATER_ABSTRACT_CREATE_H
#define LEMONADEINTERACTION_UPDATER_ABSTRACT_CREATE_H
...
protected:
    ...
    bool addSingleMonomer();

    //! function to add a monomer to a parent monomer
    bool addMonomerToParent(uint32_t parent_id);

    //! function to add a monomer at a spezific place
    bool addMonomerAtPosition(VectorInt3 pos);

    //! function to add a new monomer inbetween two others
    bool addMonomerInsideConnectedPair(uint32_t indexA, uint32_t indexB);

    //! function to add a new monomer at two others
    bool addMonomerAtConnectedPair(uint32_t indexA, uint32_t indexB);
    
    //! function to add a ring around a chain monomer
    bool addRing(uint32_t parent_id, uint32_t NRingMonomers=5);
 ...
 */
template<class IngredientsType>
bool UpdaterAbstractCreate<IngredientsType>::addSingleMonomer(){
 ...
 */
template<class IngredientsType>
bool UpdaterAbstractCreate<IngredientsType>::addMonomerToParent(uint32_t parent_id){
  // set properties of add Monomer Move
  MoveAddMonomerSc<> addmove;
  addmove.init(ingredients);
 ...
 */
template<class IngredientsType>
bool UpdaterAbstractCreate<IngredientsType>::addMonomerAtPosition(VectorInt3 position){
  MoveAddMonomerSc<> addmove;
  addmove.init(ingredients);
 ...
 */
template<class IngredientsType>
bool UpdaterAbstractCreate<IngredientsType>::addMonomerInsideConnectedPair(uint32_t indexA, uint32_t indexB){
 ...
 */
template<class IngredientsType>
bool UpdaterAbstractCreate<IngredientsType>::addMonomerAtConnectedPair(uint32_t indexA, uint32_t indexB){
  //first check if monomers are connected
  if( ! ingredients.getMolecules().areConnected(indexA,indexB))
    return false;
 ...
 */
template < class IngredientsType >
bool UpdaterAbstractCreate<IngredientsType>::addRing(uint32_t parent, uint32_t NRingMonomers)
  {
...
// 			  rng.r250_drand()>0.5 ? dz4=1 : dz4=-1;
			...
			if (PositionsFit)
			{
			  ...
			  {
			    if ( i<4  && AddRing ) if(!addMonomerAtPosition(PotentialPositions[i]) ) {AddRing=false;throw std::runtime_error("Could not add 4-monomer ring!");}
				...
				if (addMonomerInsideConnectedPair(ingredients.getMolecules().size()-(1+j%i),ingredients.getMolecules().size()-(1+(1+j)%i))) 
				{
				  AddRing=true;
				  break;
				}else { AddRing=false;}
...
#endif /* LEMONADEINTERACTION_UPDATER_ABSTRACT_CREATE_H */
