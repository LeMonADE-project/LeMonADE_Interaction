...
#ifndef LEMONADEINTERACTION_UPDATER_ADD_LINEAR_CHAINS
#define LEMONADEINTERACTION_UPDATER_ADD_LINEAR_CHAINS
/**
 * @file
 *
 ...
 * or a system with some monomers inside.
 *
...
#include <LeMonADE_Interaction/updater/UpdaterAbstractCreate.h>
...
public:
  UpdaterAddLinearChains(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, bool IsSolvent=false);
...
*/
template < class IngredientsType >
UpdaterAddLinearChains<IngredientsType>::UpdaterAddLinearChains(IngredientsType& ingredients_, uint32_t NChain_, uint32_t NMonoPerChain_, bool IsSolvent_):
BaseClass(ingredients_), NChain(NChain_), NMonoPerChain(NMonoPerChain_), density(0.0), wasExecuted(false), IsSolvent(IsSolvent_){}
